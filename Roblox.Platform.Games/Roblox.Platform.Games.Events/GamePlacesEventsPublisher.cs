...
internal class GamePlacesEventsPublisher : IGamePlacesEventsObserver, IObserver<GamePlacesChangeEvent>
{
	...
	private string Name => "GamePlacesEventsPublisher";

	public GamePlacesEventsPublisher(ILogger logger, ICounterRegistry counterRegistry)
	{
		_Logger = logger ?? throw new ArgumentNullException("logger");
		_CounterRegistry = counterRegistry;
		InitializePublisher();
		Settings.Default.MonitorChanges((Settings s) => s.AwsGamePlacesChangeTopicAccessKeyIdAndSecretCsv, InitializePublisher);
		Settings.Default.MonitorChanges((Settings s) => s.AwsGamePlacesChangeSnsTopicArn, InitializePublisher);
	}

	/// <inheritdoc />
	public void Subscribe(IObservable<GamePlacesChangeEvent> observable)
	{
	...
	/// <inheritdoc />
	public void OnNext(GamePlacesChangeEvent value)
	{
		if (Settings.Default.IsPublishToGamePlacesChangeTopicEnabled && value != null)
		{
			if (_Publisher == null)
			{
	...
	{
		VerifyAndCreatePublisher(_AccessKeyAndSecretCsv, _SnsTopicArn, Settings.Default.AwsGamePlacesChangeTopicAccessKeyIdAndSecretCsv, Settings.Default.AwsGamePlacesChangeSnsTopicArn);
	}
