...
public class AccountAddOnActivationTaskDAL
{
	private long _ID;
	...
	internal AccountAddOnActivationTaskDAL()
	{
	}
	...
	internal void Delete()
	{
		...
		EntityHelper.DoEntityDALDelete(new DbInfo(ConnectionString, "[dbo].[AccountAddOnActivationTasks_DeleteAccountAddOnActivationTaskByID]", queryParameters));
	}
	...
	internal void Insert()
	{
		...
		_ID = EntityHelper.DoEntityDALInsert<long>(new DbInfo(ConnectionString, "[dbo].[AccountAddOnActivationTasks_InsertAccountAddOnActivationTask]", new SqlParameter("@ID", SqlDbType.BigInt), queryParameters));
	}
	...
	internal void Update()
	{
		...
		queryParameters.Add(new SqlParameter("@Created", _Created));
		queryParameters.Add(new SqlParameter("@Updated", _Updated));
		EntityHelper.DoEntityDALUpdate(new DbInfo(ConnectionString, "[dbo].[AccountAddOnActivationTasks_UpdateAccountAddOnActivationTaskByID]", queryParameters));
	}

	private static AccountAddOnActivationTaskDAL BuildDAL(SqlDataReader reader)
	{
		AccountAddOnActivationTaskDAL dal = new AccountAddOnActivationTaskDAL();
		while (reader.Read())
		{
	...
	internal static AccountAddOnActivationTaskDAL Get(long id)
	{
		if (id == 0L)
		{
			return null;
		...
		return EntityHelper.GetEntityDAL(new DbInfo(ConnectionString, "[dbo].[AccountAddOnActivationTasks_GetAccountAddOnActivationTaskByID]", queryParameters), BuildDAL);
	}

	internal static EntityHelper.GetOrCreateDALWrapper<AccountAddOnActivationTaskDAL> GetOrCreate(long premiumFeatureActivationTaskID)
	{
		...
		return EntityHelper.GetOrCreateEntityDAL(new DbInfo(ConnectionString, "[dbo].[AccountAddOnActivationTasks_GetOrCreateAccountAddOnActivationTask]", queryParameters), BuildDAL);
	}
	...
	internal static ICollection<long> LeaseTasks(Guid workerId, int leaseDurationInMinutes, int maxToLease)
	{
		...
		return EntityHelper.GetDataEntityIDCollection<long>(new DbInfo(ConnectionString, "[dbo].[AccountAddOnActivationTasks_LeaseTasks]", queryParameters));
	}
}
