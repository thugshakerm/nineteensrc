...
public class AccountFeatureSetActivationTaskDAL
{
	private long _ID;
	...
	internal AccountFeatureSetActivationTaskDAL()
	{
	}
	...
	internal void Delete()
	{
		...
		EntityHelper.DoEntityDALDelete(new DbInfo(ConnectionString, "[dbo].[AccountFeatureSetActivationTasks_DeleteAccountFeatureSetActivationTaskByID]", queryParameters));
	}
	...
	internal void Insert()
	{
		...
		_ID = EntityHelper.DoEntityDALInsert<long>(new DbInfo(ConnectionString, "[dbo].[AccountFeatureSetActivationTasks_InsertAccountFeatureSetActivationTask]", new SqlParameter("@ID", SqlDbType.BigInt), queryParameters));
	}
	...
	internal void Update()
	{
		...
		queryParameters.Add(new SqlParameter("@Created", _Created));
		queryParameters.Add(new SqlParameter("@Updated", _Updated));
		EntityHelper.DoEntityDALUpdate(new DbInfo(ConnectionString, "[dbo].[AccountFeatureSetActivationTasks_UpdateAccountFeatureSetActivationTaskByID]", queryParameters));
	}

	private static AccountFeatureSetActivationTaskDAL BuildDAL(SqlDataReader reader)
	{
		AccountFeatureSetActivationTaskDAL dal = new AccountFeatureSetActivationTaskDAL();
		while (reader.Read())
		{
	...
	internal static AccountFeatureSetActivationTaskDAL Get(long id)
	{
		if (id == 0L)
		{
			return null;
		...
		return EntityHelper.GetEntityDAL(new DbInfo(ConnectionString, "[dbo].[AccountFeatureSetActivationTasks_GetAccountFeatureSetActivationTaskByID]", queryParameters), BuildDAL);
	}

	internal static EntityHelper.GetOrCreateDALWrapper<AccountFeatureSetActivationTaskDAL> GetOrCreate(long premiumFeatureActivationTaskID)
	{
		...
		return EntityHelper.GetOrCreateEntityDAL(new DbInfo(ConnectionString, "[dbo].[AccountFeatureSetActivationTasks_GetOrCreateAccountFeatureSetActivationTask]", queryParameters), BuildDAL);
	}
	...
	internal static ICollection<long> LeaseTasks(Guid workerId, int leaseDurationInMinutes, int maxToLease)
	{
		...
		return EntityHelper.GetDataEntityIDCollection<long>(new DbInfo(ConnectionString, "[dbo].[AccountFeatureSetActivationTasks_LeaseTasks]", queryParameters));
	}
}
