...
/// <summary>
/// TeamCreateEventsPublisher implements <see cref="T:Roblox.Platform.TeamCreate.Events.ITeamCreateEventsObserver" /> to get notifications of 
/// events and publishes them to a Universe entity changed SNS topic.
/// </summary>
public class TeamCreateEventsPublisher : ITeamCreateEventsObserver, IObserver<TeamCreateEvent>
{
	...
	private string Name => "TeamCreateEventsPublisher";

	public TeamCreateEventsPublisher(ILogger logger, ICounterRegistry counterRegistry)
	{
		_Logger = logger ?? throw new ArgumentNullException("logger");
		_CounterRegistry = counterRegistry;
		InitializePublisher();
		Settings.Default.MonitorChanges((Settings s) => s.AwsTeamCreateEventTopicAccessKeyIdAndSecretCsv, InitializePublisher);
		Settings.Default.MonitorChanges((Settings s) => s.AwsTeamCreateEventSnsTopicArn, InitializePublisher);
	}

	/// <inheritdoc />
	public void Subscribe(IObservable<TeamCreateEvent> observable)
	{
	...
	/// <inheritdoc />
	public void OnNext(TeamCreateEvent value)
	{
		if (Settings.Default.IsAwsTeamCreateEventPublishingEnabled && value != null)
		{
			if (_Publisher == null)
			{
	...
	{
		VerifyAndCreatePublisher(_AccessKeyAndSecretCsv, _SnsTopicArn, Settings.Default.AwsTeamCreateEventTopicAccessKeyIdAndSecretCsv, Settings.Default.AwsTeamCreateEventSnsTopicArn);
	}
